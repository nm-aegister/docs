---
title: "Threat Blocker: come attivarlo (installazione in parallelo)"
sidebarTitle: "Come Attivare (Installazione Parallelo)"
...
Questa guida descrive come collegare e attivare **Aegister Threat Blocker (ATB)**. I passaggi da 1 a 3 valgono per ogni installazione; il passaggio 4 riguarda l'installazione in parallelo al tuo firewall esistente, con istruzioni specifiche per Fortigate e SonicWall.
...
- **Preliminare:**  
  ...
  N.B. per i dispositivi già preconfigurati da Aegister per i Partner l'organizzazione è stata già creata ed è possibile registrarsi direttamente in piattaforma [qui](https://app.aegister.com) con un account aziendale registrando email e password oppure tramite SSO. 
...
- **Verifica del collegamento tra ATB e Cyber Console (Aegister):**  
...
- **IP locale ATB:**  
  Nella tua [Aegister Cyber Console](/documentation/it/acc/how-get-account) apri **Protezione Perimetrale → [Threat Blocker](https://app.aegister.com/perimeter-protection/aegister-threat-blocker/)** e fai clic sul tuo dispositivo. La pagina **Stato Threat Blocker** conferma che l'ATB è connesso (**Ultimo Aggiornamento**) e ne mostra gli indirizzi pubblico e locale nel campo **Endpoint / IP Privato**.  
  <img className="block dark:hidden" src="/images/console/threat-blocker-light.webp" alt="Stato Threat Blocker" />
  <img className="hidden dark:block" src="/images/console/threat-blocker-dark.webp" alt="Stato Threat Blocker" />
...
2. **Verifica della Connettività:**  
   ...
   ```
   http://<IP_LOCALE_ATB>:8080/blacklist.txt
   ```
...
3. **Configurazione dell'URL del Botnet Filter:**
   ...
   Sostituisci `<IP_LOCALE_ATB>` con l'IP locale del campo **Endpoint / IP Privato** nella [Aegister Cyber Console (Threat Blocker)](https://app.aegister.com/perimeter-protection/aegister-threat-blocker/), come indicato al punto 3, oppure con l'indirizzo dell'ATB noto al tuo firewall.
...
4. **Impostazione Limite degli Oggetti (Team Aegister)**
   Accedi alla pagina della Cyber Console relativa all'ATB, clicca su **Configurazione** e verifica che il campo "maximum_rules" valga 1999 (per aggirare un limite del Botnet Filter del SonicOS):
   ```json
   "maximum_rules": 1999
   ```
   In questo modo la lista resta entro il limite del Botnet Filter di SonicOS e contiene solo gli IP più pericolosi.

   <img className="block dark:hidden" src="/images/console/atb-configuration-light.webp" alt="Configurazione in Parallelo" />
   <img className="hidden dark:block" src="/images/console/atb-configuration-dark.webp" alt="Configurazione in Parallelo" />
...
6. **Abilitazione dei Log e Mirroring del Traffico:**  
   ...
   Il Log Mirroring è opzionale, a discrezione del cliente. Invia i Syslog del firewall alla dashboard di Aegister, che mostra fino agli ultimi 30 giorni di eventi in tempo reale, in forma visiva e tabellare, con un focus sugli IP del traffico in ingresso e in uscita bloccati dalla nostra Threat Intelligence.
