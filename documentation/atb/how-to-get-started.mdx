...
5. **Impostazione delle Regole e Limite degli Oggetti (Team Aegister)**
   Accedere all'ATB, cliccare su "Configuration" e verificare che il campo "maximum_rules" abbia un valore pari a 1999:
   ```json
   "maximum_rules": 1999
   ```
...
7. **Abilitazione dei Log e Mirroring del Traffico:**  
   ...
   Si ricorda che il Log Mirroring è opzionale e a discrezione del cliente. La funzione permette di inviare i log dal firewall alla dashboard aegister in modo da poter mostrare in maniera visuale e puntuale (in forma tabellare) gli eventi relativi al traffico (con evidenza dei soli indirizzi IP e delle porte) in ingresso e in uscita accettato e bloccato dalla nostra Threat Intelligence.
